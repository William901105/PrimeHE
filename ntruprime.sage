# ntruprime.sage (Python 3 / Sage 9.x+ 相容修復版)

import sys

# 輔助函式：確保在 Py2/Py3 下都能取得正確的 bytes
def to_bytes(n):
    return bytes([n])
...
def setparameters(system,random8):
  ...
  if not round1:
    ...
    # [FIX 1] 替換 chr() 為 bytes
    def Hash0(s): return Hash(to_bytes(0)+s)
    def Hash1(s): return Hash(to_bytes(1)+s)
    def Hash2(s): return Hash(to_bytes(2)+s)
    if not lpr:
      def Hash3(s): return Hash(to_bytes(3)+s)
    def Hash4(s): return Hash(to_bytes(4)+s)
    if lpr:
      def Hash5(s): return Hash(to_bytes(5)+s)
  ...
  def Short_random(): # R element with w coeffs +-1
    if round1 and lpr:
      # [FIX 1] 確保是 bytes
      k = bytes([random8() for i in range(32)])
  ...
  if lpr:
    ...
    def Expand(k):
      assert len(k) == 32
      # [FIX 2] 重大修正：
      # Sage 會將 128 視為 Sage Integer，導致 pycryptodome 底層崩潰。
      # 必須將 128 和 0 都強制轉為標準 Python int。
      s = AES.new(k, AES.MODE_CTR, counter=Counter.new(int(128), initial_value=int(0)))
      return s.encrypt(b'\0'*(4*p))
  ...
  if round1:
    ...
    def int2str(u,bytes_len):
      return bytes([(u//256^i)%256 for i in range(bytes_len)])

    def str2int(s):
      return sum(s[i]*256^i for i in range(len(s)))
