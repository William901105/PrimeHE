...
# This file was *autogenerated* from the file sntrupcore_test.sage
from sage.all_cmdline import *   # import sage library

_sage_const_32 = Integer(32); _sage_const_4 = Integer(4); _sage_const_10 = Integer(10); _sage_const_5 = Integer(5); _sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_60 = Integer(60); _sage_const_40 = Integer(40)
import os
import time
...
def to_hex(b, limit=_sage_const_32 ):
    # 確保輸入是 bytes
    if not isinstance(b, bytes):
        return f"[非 Bytes 物件: {type(b)}]"
    h = b.hex()
    if len(h) > limit:
        return h[:limit] + "..." + h[-_sage_const_4 :] + f" (Total {len(b)} bytes)"
    return h

# [FIXED] 輔助函式：將多項式轉為字串
def poly_to_str(p, limit=_sage_const_10 ):
    try:
        ...
        if len(p_list) > limit:
            # 顯示前 5 個和後 5 個係數
            start = str(p_list[:_sage_const_5 ]).replace('[', '').replace(']', '')
            end = str(p_list[-_sage_const_5 :]).replace('[', '').replace(']', '')
